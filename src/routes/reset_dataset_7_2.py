...
def start(report_id, report_json, dev):
    if "operation_period" in report_json["response"] and report_json["response"]["operation_period"] != []:
        ...
        requests_util.patch_req("Report", report_id, body={"loading": "No operation_periods Found! You need at least one Operating Period.", "is_loading_error": "yes"}, dev=dev)
        sys.exit()
    ...
    requests_util.patch_req("Report", report_id, body={"loading": "Deleting the chart data incase Air Compressors or Operating Periods have changed...", "is_loading_error": "no"}, dev=dev)
    delete_datasets_7_2(report_id, operation_period_ids, dev)

    requests_util.patch_req("Report", report_id, body={"loading": "Recreating empty charts...", "is_loading_error": "no"}, dev=dev)
...
def delete_datasets_7_2(report_id, operation_period_ids, dev):
    for operation_period_id in operation_period_ids:
        operation_json = requests_util.get_req("operation_period", operation_period_id, dev)
        ...
        if "dataset_7_2" in operation_json["response"] and operation_json["response"]["dataset_7_2"] != []:
            ...
            for dataset_7_2_id in dataset_7_2_ids:
                requests_util.del_req("dataset_7_2", dataset_7_2_id, dev)
...
def add_datasets_to_operating_periods(report_id, operation_period_ids, air_compressor_ids, dev):
    for operation_period_id in operation_period_ids: # Loop through each Operating Period
        ...
        for air_compressor_id in air_compressor_ids: # For each operating period, loop through each Air Compressor
            ...
            air_compressor_json = requests_util.get_req("air_compressor", air_compressor_id, dev)
            if "Customer CA" in air_compressor_json["response"]:
                ac_name = air_compressor_json["response"]["Customer CA"]
            else:
                requests_util.patch_req("Report", report_id, body={"loading": f"Missing Name! Air Compressor ID: {air_compressor_id}", "is_loading_error": "yes"}, dev=dev)
                sys.exit()
            
            if "Control Type" in air_compressor_json["response"]:    
                control = air_compressor_json["response"]["Control Type"]
            else:
                requests_util.patch_req("Report", report_id, body={"loading": f"Missing Control Type on Air Compressor: {ac_name}", "is_loading_error": "yes"}, dev=dev)
                sys.exit()

            if "Model" in air_compressor_json["response"]:    
                model = air_compressor_json["response"]["Model"]
            else:
                requests_util.patch_req("Report", report_id, body={"loading": f"Missing Model Number on Air Compressor: {ac_name}", "is_loading_error": "yes"}, dev=dev)
                sys.exit()
            ...
            response = requests_util.post_req("dataset_7_2", dataset_body, dev)
        ...
        requests_util.patch_req("operation_period", operation_period_id, operation_body, dev)
