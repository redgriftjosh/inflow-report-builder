...
def compile_master_df(report_id, dev):
    ...
    for idx, ac in enumerate(ac_ids):
        ...
        requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: Reading {ac_name}.csv...", "is_loading_error": "no"}, dev=dev)
        ...
        requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: {ac_name}: Kilowatts{idx+1} Column...", "is_loading_error": "no"}, dev=dev)
        ...
        requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: {ac_name}: ACFM{idx+1} Column...", "is_loading_error": "no"}, dev=dev)
        # Create ACFM Column
        ...
        if master_df is None:
            ...
            requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: Merging with other CSVs...", "is_loading_error": "no"}, dev=dev)
            print("Merged next Dataframe with master_df")
    
    if "trim" in report_json["response"] and report_json["response"]["trim"] != []:
        requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: Trimming the dataset...", "is_loading_error": "no"}, dev=dev)
        master_df = common_functions.trim_df(report_json, master_df, dev)

    if "exclusion" in report_json["response"]:
        requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: Removing Exclusions from the dataset...", "is_loading_error": "no"}, dev=dev)
    ...
    if op_per_type == "Daily":
        ...
        for operating_period_id in operating_period_ids:
            ...
            requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: Data_Crunch_{operating_period_name}.csv...", "is_loading_error": "no"}, dev=dev)
            ...
            # requests_util.patch_req("operation_period", operating_period_id, body={"data-crunch": f"https://www.pythonanywhere.com/user/jredgrift/files/home/jredgrift/inflow-report-builder/src/Data_Crunch_{common_functions.sanitize_filename(f'{operating_period_name}_{operating_period_id}')}.csv"}, dev=dev)
    ...
    requests_util.patch_req("Report", report_id, body={"loading": f"Building Data Crunch: Success!", "is_loading_error": "no"}, dev=dev)
